...
public class ANcToPicture {
    ...
    public List<NCFileProcessData> isolineProcess() throws IOException {
        ...
        if (stringObjectMap.get("status").toString().equals("200")) {
            ...
            levelProps.put(0.6,"e0f3fa");
            levelProps.put(1.2,"7ec2e9");
            levelProps.put(1.8,"4a91cc");
            levelProps.put(2.4,"2355a0");
            levelProps.put(3.0,"1c3069");
...
//                _gridData[j][k] = Double.parseDouble(String.valueOf(codeWaveData[j][k])); // 目标网省数据值
                //保证只有一位小数
                _gridData[j][k] = new BigDecimal(Double.parseDouble(String.valueOf(codeWaveData[j][k]))).setScale(1, BigDecimal.ROUND_HALF_UP).doubleValue();
            }
        }
        // 原有数据类型
    ...
    public List<Polygon> parser(Map map) {
        ...
        // 寒潮的取值范围[-16,-6]
        double[] interval = getIntervals(0, 3, 0.6);
...
//                        coordinates_h[j] = new Coordinate(ptd.X,ptd.Y);
    ...
    public static void main(String[] args) throws IOException {
        ...
        new ANcToPicture().isolineProcess();
        Date end = new Date();
