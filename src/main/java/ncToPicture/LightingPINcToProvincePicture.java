 ...
 */
@Slf4j
public class LightingPINcToProvincePicture {
    ...
    public void isolineProcess() throws IOException {
        ...
        Arrays.stream(ProvinceCoordinateEnum.values()).forEach(m -> {
            System.out.println("开始NC解析" + m.getProvinceName() + "省份！");
            try {
                // 生成等值面->List<Polygon>
                // 解析生成等值面
                map.put("xMin", m.getxMin());
                map.put("xMax", m.getxMax());
                map.put("yMin", m.getyMin());
                map.put("yMax", m.getyMax());
                map.put("province",m.getProvinceName());
                List<List<Polygon>> cPolygonList = NCInitUtils.parser(map);
                for (int i = 0; i < cPolygonList.size(); i++) {
                    //等值面结果转换
                    //将wcontour的poly转化为geojson
                    System.out.println("将"+m.getProvinceName()+"wcontour的poly转化为geojson");
                    ...
                    //将geojson输出到json文件中
                    String jsonFilePath = "C:\\Users\\JGS\\Desktop\\1\\LPIdata" + m.getProvinceName() + i + ".json";
                    ...
                    System.out.println("将"+m.getProvinceName()+"wcontour的poly转化为geojson耗时{}" + DateUtil.getCostTime(surfaceStartTimeCPolygonList, surfaceEndTimeCPolygonList));

                    //将geojson转化为shape
                    System.out.println("将"+m.getProvinceName()+"geojson转化为shape");
                    ...
                    System.out.println("将"+m.getProvinceName()+"geojson转化为shape耗时" + DateUtil.getCostTime(surfaceStartTimeShape, surfaceEndTimeShape));

                    if (stringObjectMap.get("status").toString().equals("200")) {
                        System.out.println(m.getProvinceName()+"省份开始绘图---");
...
//                levelProps.put(5.0,Color.red);
                        ...
                        //输出图片
                        String outPath = "C:\\Users\\JGS\\Desktop\\1\\lightingPI_2023030720" + m.getProvinceName() + i + ".png";
                        Map params = new HashMap();
                        double[] bbox = new double[]{
                                Double.parseDouble((String) map.get("xMin")),
                                Double.parseDouble((String) map.get("xMax")),
                                Double.parseDouble((String) map.get("yMin")),
                                Double.parseDouble((String) map.get("yMax"))};
                        ...
                        System.out.println(m.getProvinceName()+"省份绘图耗时" + DateUtil.getCostTime(surfaceStartTimePicture, surfaceEndTimePicture));
                    }
                }
            } catch (Exception e) {
                e.printStackTrace();
...
//                            log.error("NonMultiPolygon自相交待处理图形不合法不处理: {}", contourGeometry.toString());
    ...
    public static void main(String[] args) throws IOException {
        ...
        new LightingPINcToProvincePicture().isolineProcess();
        Date end = new Date();
